...
func train(fn string) map[string]int {
	...
	if w, err := ioutil.ReadFile(fn); err == nil {
		...
		for _, c := range p.FindAllString(ws, -1) {
			NWORDS[c]++
		}
	}
	return NWORDS
...
func edits1(word string, c chan string) {
	...
	for i := 0; i < len(word)+1; i++ {
		splits = append(splits, p{word[:i], word[i:]})
	}
	for _, s := range splits {
		...
		if len(s.b) > 1 {
			c <- s.a + string(s.b[1]) + string(s.b[0]) + s.b[2:]
		}
		for _, abc := range alphabet {
			if len(s.b) > 0 {
				c <- s.a + string(abc) + s.b[1:]
			}
...
func correct(word string, model map[string]int) string {
	...
	if c := best(word, edits1, model); c != "" {
		return c
	}
	if c := best(word, edits2, model); c != "" {
		return c
	}
	return word
}
